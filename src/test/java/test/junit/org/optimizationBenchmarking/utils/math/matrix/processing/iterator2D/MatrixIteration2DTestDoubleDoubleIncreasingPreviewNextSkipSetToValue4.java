...
public class MatrixIteration2DTestDoubleDoubleIncreasingPreviewNextSkipSetToValue4
    extends MatrixIteration2DTest {

  /** create */
  public MatrixIteration2DTestDoubleDoubleIncreasingPreviewNextSkipSetToValue4() {
    super();
  }
  ...
  protected void setup(final MatrixIteration2DBuilder builder) {
    builder.setEndReplacement(Double.valueOf(58.5d));
  ...
  protected void checkY(final int step, final AbstractMatrix y) {
    ...
    switch (step) {
      ...
      case 4: {
        Assert.assertEquals(y.n(), 2);
        Assert.assertEquals(33.5d, y.getDouble(0, 0), Double.MIN_VALUE);
        Assert.assertEquals(58.5d, y.getDouble(0, 1), Double.MIN_VALUE);
        return;
      }
      default: {
        throw new AssertionError("Only five steps allowed."); //$NON-NLS-1$
