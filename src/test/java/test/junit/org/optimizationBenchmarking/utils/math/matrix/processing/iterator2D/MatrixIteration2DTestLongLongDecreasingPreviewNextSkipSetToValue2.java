...
public class MatrixIteration2DTestLongLongDecreasingPreviewNextSkipSetToValue2
    extends MatrixIteration2DTest {

  /** create */
  public MatrixIteration2DTestLongLongDecreasingPreviewNextSkipSetToValue2() {
    super();
  }
  ...
  protected void setup(final MatrixIteration2DBuilder builder) {
    builder.setEndReplacement(Long.valueOf(-321L));
  ...
  protected void checkY(final int step, final AbstractMatrix y) {
    ...
    switch (step) {
      ...
      case 7: {
        Assert.assertEquals(y.n(), 4);
        Assert.assertEquals(-321L, y.getLong(0, 0));
        Assert.assertEquals(600L, y.getLong(0, 1));
        Assert.assertEquals(9L, y.getLong(0, 2));
        Assert.assertEquals(8L, y.getLong(0, 3));
        return;
      }
      case 8: {
        Assert.assertEquals(y.n(), 4);
        Assert.assertEquals(-321L, y.getLong(0, 0));
        Assert.assertEquals(-321L, y.getLong(0, 1));
        Assert.assertEquals(9L, y.getLong(0, 2));
        Assert.assertEquals(8L, y.getLong(0, 3));
        return;
      }
      case 9: {
        ...
        Assert.assertEquals(-321L, y.getLong(0, 1));
        Assert.assertEquals(9L, y.getLong(0, 2));
        Assert.assertEquals(-321L, y.getLong(0, 3));
        return;
      }
      default: {
        throw new AssertionError("Only ten steps allowed."); //$NON-NLS-1$
