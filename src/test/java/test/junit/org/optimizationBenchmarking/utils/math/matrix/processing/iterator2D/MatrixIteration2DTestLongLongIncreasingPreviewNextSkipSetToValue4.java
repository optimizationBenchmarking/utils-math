...
public class MatrixIteration2DTestLongLongIncreasingPreviewNextSkipSetToValue4
    extends MatrixIteration2DTest {

  /** create */
  public MatrixIteration2DTestLongLongIncreasingPreviewNextSkipSetToValue4() {
    super();
  }
  ...
  protected void setup(final MatrixIteration2DBuilder builder) {
    builder.setEndReplacement(Long.valueOf(58L));
  ...
  protected void checkY(final int step, final AbstractMatrix y) {
    ...
    switch (step) {
      ...
      case 4: {
        Assert.assertEquals(y.n(), 2);
        Assert.assertEquals(33L, y.getLong(0, 0));
        Assert.assertEquals(58L, y.getLong(0, 1));
        return;
      }
      default: {
        throw new AssertionError("Only five steps allowed."); //$NON-NLS-1$
