  ...
  private final void __setX(final double x) {
    ...
    outer: for (index = 0; index < this.m_indexes.length; index++) {
      ...
      looper: for (; position < max; position++) {
        if (Compare.compare(MatrixIterator2D._d(//
      ...
      this.m_y[have] = val = MatrixIterator2D
          ._d(matrix.getDouble(position, this.m_yDim));
  ...
  final void _findNext() {
    ...
    for (index = this.m_indexes.length; (--index) >= 0;) {
      ...
      xAtPosition = MatrixIterator2D
          ._d(matrix.getDouble(oldPosition, xDim));
      yAtPosition = yAtOldPosition = MatrixIterator2D
          ._d(matrix.getDouble(oldPosition, yDim));
      ...
      inner: for (;;) {
        if ((xAtPosition > oldX) && //
            (noSmallestLarger || //
                (Compare.compare(xAtPosition, smallestLarger) < 0))) {
          ...
          if ((position <= 0)//
              || (Compare.compare(yAtPosition, yAtOldPosition) != 0)//
    ...
    if (noSmallestLarger) {
      ...
      for (final IMatrix matrixx : this.m_matrices) {
        ...
        if (position > 0) {
          xAtPosition = MatrixIterator2D
              ._d(matrixx.getDouble((position - 1), xDim));
          if (Compare.compare(xAtPosition, smallestLarger) > 0) {
            smallestLarger = xAtPosition;
            noSmallestLarger = false;
          }
        }
